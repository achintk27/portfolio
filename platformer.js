...
function start(){started=true;welcome.hidden=true;document.getElementById('adventure').classList.add('playing');canvas.focus({preventScroll:true});}
...
function openWorld(id){if(id==='projects'){visited.add(id);document.getElementById('visit-count').textContent=`${visited.size} / 4`;clearInput();document.getElementById('projects').scrollIntoView({behavior:'smooth'});return;}activeWorld=id;player.vx=0;player.vy=0;const source=document.getElementById(id);if(!source)return;keys.clear();near=null;enter.hidden=true;document.getElementById('world-content').replaceChildren(source.cloneNode(true));document.querySelectorAll('#world-content [id]').forEach(n=>n.removeAttribute('id'));document.getElementById('world-dialog-title').textContent=id==='contact'?'LET’S CONNECT':`WORLD ${stops.findIndex(s=>s.id===id)+1} · ${id.toUpperCase()}`;if(stops.some(s=>s.id===id)){visited.add(id);document.getElementById('visit-count').textContent=`${visited.size} / 4`;}document.querySelectorAll('[data-world]').forEach(b=>{b.classList.toggle('explored',visited.has(b.dataset.world));b.setAttribute('aria-label',`${b.textContent}${visited.has(b.dataset.world)?', explored':''}`);});const index=stops.findIndex(s=>s.id===id);document.getElementById('previous-world').disabled=index<=0;document.getElementById('next-world').disabled=index<0||index===stops.length-1;if(!dialog.open)dialog.showModal();dialog.scrollTop=0;}
...
mode.addEventListener('click',()=>{clearInput();document.getElementById('projects').scrollIntoView({behavior:'smooth'});});
const mapDialog=document.getElementById('map-dialog');
document.getElementById('show-map').addEventListener('click',()=>{clearInput();mapDialog.showModal();});
document.getElementById('close-map').addEventListener('click',()=>mapDialog.close());
document.querySelectorAll('[data-travel]').forEach(button=>button.addEventListener('click',()=>{mapDialog.close();const stop=stops.find(s=>s.id===button.dataset.travel);start();player.x=stop.x-70;player.y=floor-player.h;player.vy=0;if(stop.id==='projects')openWorld('projects');}));
window.addEventListener('keydown',e=>{if(!document.body.classList.contains('game-mode')||dialog.open||mapDialog.open||!started||['BUTTON','A','INPUT','TEXTAREA'].includes(document.activeElement?.tagName))return;const k=e.key.toLowerCase();if(['arrowleft','arrowright','arrowup',' ','a','d','w','e','enter'].includes(k)){e.preventDefault();keys.add(k);if([' ','w','arrowup'].includes(k)&&!e.repeat)jump();if((k==='e'||k==='enter')&&near&&!e.repeat)openWorld(near.id);}});window.addEventListener('keyup',e=>keys.delete(e.key.toLowerCase()));window.addEventListener('blur',clearInput);
...
// Layered hills and landmarks make the world feel wider without heavy 3D assets.
const sky=ctx.createLinearGradient(0,0,0,480);sky.addColorStop(0,'#54aeff');sky.addColorStop(1,'#d6f6ff');ctx.fillStyle=sky;ctx.fillRect(0,0,width,480);
for(let i=0;i<12;i++){const x=i*320-camera*.25;ctx.fillStyle=i%2?'#a3dbc4':'#83cfb4';ctx.beginPath();ctx.ellipse(x,410,170,190+(i%3)*35,0,Math.PI,0);ctx.fill();ctx.fillStyle='#d4f2d8';ctx.fillRect(x-40,285,12,35);ctx.fillRect(x+35,320,12,24);}
for(let i=0;i<15;i++){const x=i*240-camera*.55;ctx.fillStyle='#56b97c';ctx.beginPath();ctx.ellipse(x,420,160,90,0,Math.PI,0);ctx.fill();}
...
for(const s of stops){const colors=['#ed6754','#9072dd','#439ddb','#f2b83f'];const i=stops.indexOf(s);ctx.fillStyle=colors[i];ctx.fillRect(s.x+100,300,115,100);ctx.fillStyle='#fff9df';ctx.fillRect(s.x+114,317,86,48);ctx.fillStyle='#213d68';ctx.fillRect(s.x+145,370,30,30);text(['HELLO!','NYU / PES','WORKSHOP','PROJECT LAB'][i],s.x+158,345,11,'#213d68');ctx.fillStyle=colors[i];ctx.beginPath();ctx.moveTo(s.x+85,300);ctx.lineTo(s.x+158,252);ctx.lineTo(s.x+230,300);ctx.fill();}
...
const reportAssetError=()=>{document.getElementById('game-announcement').textContent='Some artwork could not load. Reload to retry. All projects remain available below.';};
const sheet=new Image();sheet.onerror=reportAssetError;sheet.onload=()=>tiles=sheet;sheet.src='assets/tilemap.png';const hero=new Image();hero.onerror=reportAssetError;hero.onload=()=>character=hero;hero.src='assets/boy-sprite.png';
})();
